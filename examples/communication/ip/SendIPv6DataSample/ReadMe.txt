  Introduction
  ------------
  This sample Java application shows how to send IPv6 data from an IPv6 device to 
  another one connected to a network using the XBee Java Library.

  The application sends IPv6 data to another Thread device on the network with a 
  ...
  Files
  -----
    * com.digi.xbee.api.sendipv6data.MainApp.java:
      Main application class. It instantiates a Thread device, establishes a 
      serial connection with it and sends the IPv6 data to the XBee device with 
  ...
  -------------
    ...
    4) Set the destination IPv6 address and port number in the MainApp class.
       You can find them by reading the 'MY' ('GA' or 'LA') and 'C0' settings 
       of the destination XBee device with XCTU.
  ...
  Finally, launch the sample application, some IPv6 data is sent to the configured 
  remote XBee device. When that happens, a line with the result of the operation 
  is printed to the standard output:

    Sending data to XXXX:XXXX:XXXX:XXXX:XXXX:XXXX:XXXX:XXXX >> 48 65 6C 6C 6F 20 58 42 65 65 21 | Hello XBee!... Success
