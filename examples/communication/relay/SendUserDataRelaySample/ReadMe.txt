  Introduction
  ------------
  This sample Java application shows how to send data to other interfaces of the
  XBee device using the XBee Java Library.

  The application sends 10 User Data Relay messages to Bluetooth Low Energy
  interface.
  ...
  Files
  -----
    * com.digi.xbee.api.senduserdatarelay.MainApp.java:
      Main application class. It instantiates an XBee device, establishes a
      serial connection with it and sends the data to the specified XBee
      interface. Finally, it prints out the result of the sent operation.
  ...
  Finally, launch the sample application. A total of 10 User Data Relay messages
  are sent to the Bluetooth interface. When that happens, a line with the result
  of each operation is printed to the standard output:

    Sending User Data Relay to Bluetooth Low Energy >> 'Hello from the serial interface (#1)'... Success
    Sending User Data Relay to Bluetooth Low Energy >> 'Hello from the serial interface (#2)'... Success
    Sending User Data Relay to Bluetooth Low Energy >> 'Hello from the serial interface (#3)'... Success
    ...
