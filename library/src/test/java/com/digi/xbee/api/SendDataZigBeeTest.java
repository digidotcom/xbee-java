...
public class SendDataZigBeeTest {
	...
	public void setup() throws Exception {
		...
		// Mock a RemoteZigBeeDevice to be used as parameter in the send data command.
	...
	public void testSendDataZigBeeInvalidParams() {
		...
		try {
			zigbeeDevice.sendData((XBee64BitAddress)null, XBEE_16BIT_ADDRESS, SEND_DATA_BYTES);
		...
		try {
			zigbeeDevice.sendData(XBEE_64BIT_ADDRESS, (XBee16BitAddress)null, SEND_DATA_BYTES);
			fail("Data shouldn't have been sent successfully.");
		} catch (Exception e) {
			assertEquals(NullPointerException.class, e.getClass());
		}
		...
		try {
			zigbeeDevice.sendData((XBee64BitAddress)null, (XBee16BitAddress)null, SEND_DATA_BYTES);
			fail("Data shouldn't have been sent successfully.");
		} catch (Exception e) {
			assertEquals(NullPointerException.class, e.getClass());
		}
		...
		try {
			zigbeeDevice.sendData((RemoteZigBeeDevice)null, SEND_DATA_BYTES);
			fail("Data shouldn't have been sent successfully.");
		} catch (Exception e) {
			assertEquals(NullPointerException.class, e.getClass());
		}
		...
		try {
			zigbeeDevice.sendData(XBEE_64BIT_ADDRESS, XBEE_16BIT_ADDRESS, null);
			fail("Data shouldn't have been sent successfully.");
		} catch (Exception e) {
			assertEquals(NullPointerException.class, e.getClass());
		}
		...
		try {
			zigbeeDevice.sendData(mockedRemoteDevice, null);
			fail("Data shouldn't have been sent successfully.");
		} catch (Exception e) {
			assertEquals(NullPointerException.class, e.getClass());
		} 
	...
	public void testSendDataZigBeeSuccess() throws Exception {
		...
		PowerMockito.doReturn(transmitStatusSuccess).when(zigbeeDevice, SEND_XBEE_PACKET_METHOD, Mockito.eq(transmitPacket));
		...
		// Verify the sendXBeePacket method was called 2 times (one for each data send).
		PowerMockito.verifyPrivate(zigbeeDevice, Mockito.times(2)).invoke(SEND_XBEE_PACKET_METHOD, (XBeeAPIPacket)Mockito.any());
	}
	...
	public void testSendDataZigBeeTxStatusError() throws Exception {
		...
		PowerMockito.doReturn(transmitStatusError).when(zigbeeDevice, SEND_XBEE_PACKET_METHOD, Mockito.eq(transmitPacket));
	...
	public void testSendDataZigBeeTimeout() throws Exception {
		...
		PowerMockito.doThrow(new TimeoutException()).when(zigbeeDevice, SEND_XBEE_PACKET_METHOD, Mockito.eq(transmitPacket));
	...
	public void testSendDataZigBeeIOError() throws Exception {
		...
		PowerMockito.doThrow(new IOException()).when(zigbeeDevice, SEND_XBEE_PACKET_METHOD, Mockito.eq(transmitPacket));
